...
참고삼아 원본 프로젝트 근황도 적어둡니다. 지금은 아침마다 증권사 API로 실계좌 스냅샷을 받아 웹 대시보드에 그리고, 페이퍼트레이딩은 GitHub Actions에서 알아서 쌓입니다. 거래가 생기면 그 시점의 차트를 박제해서 매매일지에 남기고, 시장 국면과 종목 등급도 매일 계산합니다. 실전 주문은 아직 안 붙였습니다. 백테스트를 통과한 전략이라도 모의 운용을 한 번 더 통과해야 실전 후보가 된다는 규칙을 세워놨고, 그 전까지 실제 돈은 들어가지 않습니다.
